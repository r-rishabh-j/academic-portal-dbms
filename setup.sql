...
values ('btech', 6, 10); -- bachelors degree only
...
BEGIN
	...
	select usesuper from pg_user where usename = curr_user into is_super;
	if is_super=false then
	IF curr_user!=NEW.course_coordinator THEN raise notice 'Invalid faculty id';RETURN NULL; END IF;end if;
...
--    execute(format($f$create TABLE $f$));
    ...
    close student_cursor;
   	 ...
   	 ----------------
       execute(format($d$create TABLE registrations.drop_tickets_%s_%s (roll_number varchar, course_code varchar,PRIMARY KEY(roll_number,course_code));$d$,academic_year,semester_number));
       execute(format($d$create TABLE registrations.withdraw_course_%s_%s (roll_number varchar, course_code varchar,PRIMARY KEY(roll_number,course_code));$d$,academic_year,semester_number));
       execute(format($d$create TRIGGER drop_ticket_trigger_%s_%s BEFORE INSERT ON registrations.drop_tickets_%s_%s FOR EACH ROW EXECUTE FUNCTION check_drop_request();$d$,academic_year,semester_number,academic_year,semester_number));
       execute(format($d$create TRIGGER withdraw_course_trigger_%s_%s BEFORE INSERT ON registrations.withdraw_course_%s_%s FOR EACH ROW EXECUTE FUNCTION check_withdraw_request();$d$,academic_year,semester_number,academic_year,semester_number)); 
      ------------  
      ...
      open student_cursor;
        loop
            ...
            execute format('grant select, insert on registrations.faculty_ticket_'||f_id||'_'||academic_year||'_' || semester_number ||' to '||s_rollnumber||';');
           	-----
           	execute(format($d$grant SELECT, INSERT ON registrations.drop_tickets_%s_%s TO %s;$d$,academic_year, semester_number, s_rollnumber));
          	execute(format($d$grant SELECT, INSERT ON registrations.withdraw_course_%s_%s TO %s;$d$,academic_year, semester_number,s_rollnumber));
          	-----
        end loop;
        close student_cursor;
    end loop;
