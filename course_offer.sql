create or replace procedure faculty_actions.offer_course(course_name text, instructor_list text[], allowed_batches text[], cgpa_lim real)
as
$proc$
declare
...
begin
    ...
    execute(format($d$insert into course_offerings.sem_%s_%s values('%s','%s','%s',null,'%s',%s)$d$, course_name, curr_user, instructor_list, allowed_batches,cgpa_lim));
